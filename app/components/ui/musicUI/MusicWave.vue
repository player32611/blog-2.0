...
const { musicCurrentRef, isPlaying } = useSoundMusic();
...
const getWavesData = async (length: number, points: number) => {
	if (!musicCurrentRef.value) return;
	const response = await fetch(musicCurrentRef.value?.path);
...
<template>
	<div class="music_wave">
		<div
			v-for="i in linesTotal"
			class="wave_line"
			:key="i"
			:style="`transform: rotate(${(360 / linesTotal) * i}deg); --s: 0;`"
			:ref="el => setLinesRef(el)"
		></div>
	</div>
</template>

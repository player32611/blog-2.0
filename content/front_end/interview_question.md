...
> JavaScript 中每个对象内部都有一个 `[[Prototype]]`，它指向另一个对象，这个对象就是它的原型。函数对象还有一个 prototype 属性，它通常作为通过 new 创建实例的原型对象。

> 当访问一个对象的属性或方法时，会先查找对象自身，如果找不到，就会沿着 `[[Prototype]]` 向上查找，直到找到目标属性或者到达 null。这种由多个原型对象连接形成的链式结构就是原型链。

> 原型主要用于实现属性和方法的共享，而原型链是 JavaScript 实现对象继承和属性查找的重要机制。

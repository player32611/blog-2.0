...
算法思想：每次找出未排序序列中最小的元素，然后放进有序序列的后面。
...
算法思想：执行 n-1 趟操作，每趟从前往后比较待排序区间的相邻元素，如果逆序，就交换。每趟结束之后，就会有一个较大元素在最终的位置上。
...
求窗口内最大值：双端队列（单调递减） + 贪心

::

::tip

求窗口内最小值：双端队列（单调递增） + 贪心
...
解法：把所有字符串利用“字符串哈希”映射成数之后，看看一共有多少个不同的数
